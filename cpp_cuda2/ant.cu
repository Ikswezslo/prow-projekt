...
void cuda_ants_update(int n, double* result, int* solutions, int ants) {
    ants_update<<<ants,n, n*sizeof(CityInfo)>>>(d_array, d_n, d_pheromone_influence, d_length_influence, d_result, d_solutions, d_random);
...
__global__ void ants_update(Road *matrix, int* n, 
                       ...
                       curandState *states) {
    extern __shared__ CityInfo sharedData[];
    int id = blockIdx.x;
	int seed = id;
    curand_init(seed, id, 0, &states[id]);
    curandState* state = &states[id];
    __shared__ AntData data[1];
    if (threadIdx.x == 0) {
        data->n = *n;
    ...
    __syncthreads();
    int temp_selected_city;
    for(int i = 1; i < data->n; i++) {
        temp_selected_city = select_next_city(matrix, data, state);
        if (threadIdx.x == 0) {
            data->actual_city = temp_selected_city;
            sharedData[data->actual_city].is_visited = true;
...
__device__ void calculate_probabilities(Road* matrix, AntData* data) {
    extern __shared__ CityInfo sharedData[];
    float candidate_probability;
    if(!(sharedData[threadIdx.x].is_visited)) {
        candidate_probability = powf(matrix[data->actual_city * data->n + threadIdx.x].pheromone,data->pheromone_influence) * powf(1/matrix[data->actual_city * data->n + threadIdx.x].length,data->length_influence);
        sharedData[threadIdx.x].probability = candidate_probability;
    }
    else {
        sharedData[threadIdx.x].probability = 0;
    }
}
...
__device__ int select_next_city(Road* matrix, AntData* data, curandState *state) {
    ...
    __syncthreads(); // ???
    calculate_probabilities(matrix, data);
    __syncthreads();
    ...
    if (threadIdx.x == 0) {
        ...
        for(int i = 0; i < blockDim.x; i++) 
            sum_probability += sharedData[i].probability;

        for(int i = 0; i < blockDim.x; i++) 
            sharedData[i].probability /= sum_probability;

        float cumulative_probability = 0;
        
        float random = curand_uniform(state);
        for(int i = 0; i < blockDim.x; i++){
            cumulative_probability += sharedData[i].probability;
